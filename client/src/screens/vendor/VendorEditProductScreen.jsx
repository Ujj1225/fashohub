...
const VendorEditProductScreen = () => {
  ...
  const getSecondaryOptions = () => {
    if (primaryCategory && categoriesData[primaryCategory]) {
  ...
  useEffect(() => {
    if (product) {
      ...
      setOff(product.off || "");
      setSizes(product.sizes || [{ size: "", quantity: "" }]);
  ...
  return (
    <div className="py-14 flex flex-col items-center w-[75%] md:w-full md:px-2">
      ...
      {loadingUpdate && (
        <p className="text-lg font-medium">Updating product...</p>
      )}
      {isLoading && (
        <p className="text-lg font-medium">Loading product details...</p>
      )}
      {error && <p className="text-xs font-thin">{error?.data?.message}</p>}
      <form
      ...
      >
        ...
        <button
          className="bg-red-500 px-4 py-1 rounded-lg hover:bg-red-600 text-white font-bold ml-4"
          onClick={() => {
            navigate("/vendor");
          }}
        >
