    ...
    <>
      ...
      <div className="w-[25%] bg-blue-500 p-4 min-h-screen block md:hidden">
        <Link to="/admin" className="flex items-center">
          ...
          <p className="ml-4 text-4xl font-extrabold text-white">Admin</p>
        </Link>
        {adminNavData.map((item, index) => (
          <Link
            key={index}
            to={`/admin/${item.replace(/\s+/g, "-").toLowerCase()}`}
          >
      ...
      {showNav && (
        <div className="md:fixed top-0 left-0 md:w-[75%] h-full bg-blue-500 p-4 z-40">
          <Link to="/admin" className="flex items-center md:mt-20">
            <img src={Logo} alt="Logo" className="w-12 h-12" />
            <p className="ml-4 text-3xl font-extrabold text-white">Admin</p>
          </Link>
          {adminNavData.map((item, index) => (
            <Link
              key={index}
